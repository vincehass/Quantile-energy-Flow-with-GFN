...
class Molecule:
    ...
    def get_representation(self):
        ...
        return torch.tensor([ord(char) for char in self.sequence], dtype=torch.float32, device=device)
...
class DistributionalGFlowNet:
    def __init__(self, input_size):
        self.policy_net = MoleculeGeneratorNetwork(input_size, 4).to(device)
        self.flow_net = MoleculeGeneratorNetwork(input_size, 2).to(device)
