...
# Check for GPU support in MLX and set the device accordingly
device = mx.device("gpu") if mx.is_available() else mx.device("cpu")
...
class Molecule:
    ...
    def get_representation(self):
        ...
        return mx.tensor([ord(char) for char in self.sequence], dtype=mx.float32, device=device).unsqueeze(0)  # Add batch dimension
...
class TransformerNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.embedding(x)
        x = self.transformer(x, x)  # Use the same input for source and target for simplicity
...
class DistributionalGFlowNet:
    ...
    def sample_action(self, state):
        ...
        action_probs = mx.softmax(logits, dim=1)  # Convert to probabilities
        action = mx.multinomial(action_probs.squeeze(), 1).item()  # Sample an action based on the probabilities
        return action

    def compute_loss(self, trajectories, rewards):
        total_loss = 0
        for trajectory, reward in zip(trajectories, rewards):
            state_vectors = mx.stack(trajectory).to(device)
            flow_preds = self.flow_net(state_vectors)
            flow_means = flow_preds[:, 0]
            flow_vars = mx.exp(flow_preds[:, 1])  # Variance should be positive, so we apply exp
